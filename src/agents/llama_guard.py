...
class LlamaGuard:
    def __init__(self) -> None:
        ...
        # Try to use GROQ LlamaGuard first
        if settings.GROQ_API_KEY is not None:
            try:
                self.model = get_model(GroqModelName.LLAMA_GUARD_3_8B).with_config(tags=["skip_stream"])
                print("Using GROQ LlamaGuard model")
            except Exception as e:
                print(f"Error loading GROQ LlamaGuard: {e}")
                self.model = None
        
        # If GROQ not available or failed, try using OpenAI
        if self.model is None and settings.OPENAI_API_KEY is not None:
            try:
                # Use GPT-4o or GPT-4o-mini as they're good at following the LlamaGuard format instructions
                model_name = OpenAIModelName.GPT_4O if OpenAIModelName.GPT_4O in settings.AVAILABLE_MODELS else OpenAIModelName.GPT_4O_MINI
                self.model = get_model(model_name).with_config(tags=["skip_stream"])
                print(f"Using OpenAI {model_name} for content moderation")
            except Exception as e:
                print(f"Error loading OpenAI model: {e}")
                self.model = None
        
        # If still no model, log a clear message
        if self.model is None:
            print("No suitable model (GROQ or OpenAI) available for content moderation")
